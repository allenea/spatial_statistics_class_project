...
for case_time in casestudy_time:
    ...
    data_file = data_dir+"/Verification_Data/hr_avg_trim/"+dtype_subusage[0]+"/"+case_time[:10]+"/"
    outdir =  data_dir+"/Verification_Data/interpolation_pre/"+dtype_subusage[0]+"/"+case_time[:10]+"/"
    ...
    for dtype in dtype_subusage:
        ...
        filelist = glob.glob(data_file+"/"+'*.csv')
        lenlst = len(filelist)
        print("expected length = ",lenlst*49)
    ...
    blankMatrix[:,1] = [int(mo) for mo in avg_month]
    ...
    outstring = outdir + "all_hr_avg_trim.csv"
    print(outstring)
    ...
    df2 = df.sort_values(by=['YEAR', 'MONTH','DAY', 'HOUR','MINUTE'])  
    df2.to_csv(outstring, index=False)
    
    del blankMatrix,data,df,df2  ## Comment out for debugging
    #break
